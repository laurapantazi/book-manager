...
<template>
  <div class="books__wrapper">
    ...
    <section class="search-options">
      ...
      <div>
        <button class="btn btn-outline-secondary fa-button btn-sm" style="float: right" @click="showFiltersFlag = !showFiltersFlag"><i class="fa fa-filter" aria-hidden="true"></i>Filter</button>
      </div>
    </section>
    ...
    <section class="page">
      ...
      <div class="page__list">
        <ul class="pagination">
          <li class="page-item page-section__previous">
            <a class="page-link" @click="changePage(0, 'previous')">Previous</a>
          </li>
          ...
          <li class="page-item page-section__next">
            <a class="page-link" @click="changePage(0, 'next')">Next</a>
          </li>
        </ul>
      </div>
    </section>
...
export default {
  ...
  mounted() {
    ...
    if (token) {
      ...
      this.$axios
        .post(ENDPOINT + 'books/paginate/count', {
          offset: this.offset,
          limit: this.limit
        })
        .then(response => {
  ...
  methods: {
    ...
    searchBooks() {
      this.offset = 0
      this.$axios
        .post(ENDPOINT + 'books/search/paginate/count', {
          offset: this.offset,
          limit: this.limit,
          search_array: this.search
        })
        .then(response => {
          this.books = response.data.books
          this.totalBooks = response.data.count
    ...
    paginate(offset, limit) {
      this.$axios
        .post(ENDPOINT + 'books/paginate', { offset, limit })
        .then(response => {
          this.books = response.data.books
        })
        .catch(error => {
